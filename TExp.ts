...
;; [Empty -> void]
*/
import {chain, concat, filter, map, uniq, reduce} from "ramda";
...
const sortMe = (texpList: TExp[]): TExp[] => {
    return texpList.sort((a:TExp, b:TExp) => a.tag < b.tag? -1 : a.tag === b.tag ? 0 : 1);
};
...
const flat = (arr:any[]):any[] =>{
    return reduce((acc:any[], curr)=> isArray(curr)? acc.concat(curr) : acc.concat([curr]), [],arr);

};
const validateNoError = (x:any[]):Error | any[] =>{
...
const unparseArrayNoError=(x: any[]):string | Error=>{
    return isEmpty(filter(isError, x))? x.join(" | "): Error("Error occoured!");
};
                            ...
                            isEmptyTVar(x) ? x.var :
                                isTVar(x) ? up(tvarContents(x)) :
                                    isProcTExp(x) ? [...unparseTuple(x.paramTEs), '->', unparseTExp(x.returnTE)] :
                                        isUnionTExp(x) ? unparseArrayNoError(map((y:TExp):string | Error => unparseTExp(y), x.params)):
                                        ["never"];
